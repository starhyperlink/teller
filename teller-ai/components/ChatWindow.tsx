...
export default function ChatWindow() {
  ...
  const hasIntroGreeting =
    messages[0]?.role === "assistant" &&
    messages[0]?.content.includes("Hi, I’m Teller AI");
  ...
  function PaperclipIcon() {
    return (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4">
        <path d="M21.44 11.05l-8.49 8.49a5.5 5.5 0 0 1-7.78-7.78l9.9-9.9a4 4 0 1 1 5.66 5.66l-9.2 9.21a2.5 2.5 0 0 1-3.54-3.54l8.18-8.18" />
      </svg>
    );
  }
      ...
      <main className="flex flex-1 flex-col">
          ...
          <div className="mx-auto max-w-3xl space-y-4">
            {isFreshChat && (
              <div className="flex justify-center pt-2">
                <div className="max-w-xl rounded-full border border-white/10 bg-white/5 px-6 py-3 text-center shadow-[0_10px_30px_rgba(0,0,0,0.25)] backdrop-blur-sm">
                  <p className="text-sm font-medium text-white/90">Hi, I’m Teller AI.</p>
                  <p className="mt-1 text-sm text-neutral-300">Ask me anything — I can help with research, writing, business, coding, summaries, and ideas.</p>
                </div>
              </div>
            )}

            {messages
              .filter((message, index) => !(isFreshChat && index === 0 && message.role === "assistant"))
              .map((message, index) => (
        ...
        <div className="border-t border-neutral-800 p-4">
          <div className="mx-auto max-w-3xl">
            {hasSession ? (
              <div className="flex items-center gap-2 rounded-full border border-neutral-700 bg-neutral-900 px-2 py-2 shadow-[0_12px_30px_rgba(0,0,0,0.28)]">
                <label className="flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-full bg-neutral-800 text-neutral-200 transition hover:bg-neutral-700" title="Attach file">
                ...
                {pendingFile && (
                  <div className="flex items-center gap-2 rounded-full border border-neutral-700 bg-neutral-800/80 px-2.5 py-1.5 text-xs text-neutral-200 max-w-[38%]">
                    <span className="truncate">{pendingFile.name}</span>
                    <button type="button" onClick={() => setPendingFile(null)} className="rounded-full bg-neutral-700 px-1.5 py-0.5 text-[10px] text-white">✕</button>
                  </div>
                )}

                <input className="min-w-0 flex-1 border-0 bg-transparent px-3 py-2 text-sm text-white placeholder:text-neutral-400 focus:outline-none" placeholder="Ask Teller AI anything..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") sendMessage(); }} />

                <button onClick={sendMessage} disabled={loading} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-black transition hover:bg-neutral-200 disabled:opacity-50" aria-label="Send message">
                  <SendIcon />
                </button>
              </div>
            ) : (
